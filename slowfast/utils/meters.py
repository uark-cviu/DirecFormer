...
class TestMeter(object):
    ...
    def update_stats(self, preds, labels, clip_ids):
        ...
        for ind in range(preds.shape[0]):
            ...
            if self.path_to_videos is not None:
                vid_path = os.path.dirname(self.path_to_videos[int(clip_ids[ind])][0])
                out = f'{vid_path} {int(np.argmax(preds[ind]))} {int(labels[ind])}'
...
class TrainMeter(object):
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        self.net_timer = Timer()
        self.loss = ScalarMeter(cfg.LOG_PERIOD)
        self.perm_loss = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_total = 0.0
        self.lr = None
    ...
    def update_stats(self, top1_err, top5_err, loss, lr, mb_size, perm_loss=0):
        """
        Update the current stats.
        Args:
        ...
        """
        self.loss.add_value(loss)
        self.perm_loss.add_value(perm_loss)
        self.lr = lr
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            ...
            "eta": eta,
            "loss": self.loss.get_win_median(),
            "perm_loss": self.perm_loss.get_win_median(),
            "lr": self.lr,
            "gpu_mem": "{:.2f}G".format(misc.gpu_mem_usage()),
        }
        if not self._cfg.DATA.MULTI_LABEL:
