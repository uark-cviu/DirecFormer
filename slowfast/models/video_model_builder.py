...
from slowfast.models.indices_model import IndicesHead
from slowfast.models.vivit import Transformer
...
class SlowFast(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        # version 1: frame_rate = 8, upsample = 2
        # version 2: frame_rate = 1, upsample = 2
        frame_rate = 1
        upsample = 2
        if self.enable_puzzle:
            self.indices_model = IndicesHead(
                dim_in=width_per_group * 32 // cfg.SLOWFAST.BETA_INV,
                dim_out=cfg.DATA.NUM_FRAMES // frame_rate,
                pool_size=[
                    cfg.DATA.NUM_FRAMES // pool_size[-1][0],
                    cfg.DATA.TRAIN_CROP_SIZE // 32 // pool_size[-1][1],
                    cfg.DATA.TRAIN_CROP_SIZE // 32 // pool_size[-1][2],
    ...
    def forward(self, x, bboxes=None):
        ...
        x = self.s5(x)
        if self.enable_puzzle:
            indices = self.indices_model(x[-1])
...
class ResNet(nn.Module):
    ...
    def _construct_network(self, cfg):
        ...
        if self.enable_puzzle:
            self.indices_model = IndicesHead(
                dim_in=width_per_group * 32,
                dim_out=cfg.DATA.NUM_FRAMES,
                pool_size=[
                    cfg.DATA.NUM_FRAMES // pool_size[0][0],
                    cfg.DATA.TRAIN_CROP_SIZE // 32 // pool_size[0][1],
                    cfg.DATA.TRAIN_CROP_SIZE // 32 // pool_size[0][2],
                ],
                upsample=pool_size[0][0],
            )
...
class ViViT(nn.Module):
    ...
    def forward(self, x):
        x = x[0].permute(0, 2, 1, 3, 4) # N C T H W -> N T C H W
