...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., with_qkv=True, attention_type=None):
        ...
        if self.attention_type == 'divided_space_time':
            ...
            self.cos.register_forward_hook(self.save_output_hooks())
    ...
    def forward(self, x):
        ...
        if self.attention_type == 'divided_space_time':
            attn = self.cos(q*self.scale, k*self.scale)
        else:
    ...
    def save_output_hooks(self) -> Callable:
        def fn(_, __, output):
            self.attn_feature = output
        return fn
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if self.attention_type == 'divided_space_time':
            ...
            self.temporal_attn.register_forward_hook(self.save_output_hooks())
    ...
    def save_output_hooks(self) -> Callable:
        def fn(module, module_in, module_out):
            self.attn_feature = module.attn_feature
        return fn
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        ## Attention Blocks
        self.attn_feature = [torch.empty(0).cuda() for i in range(self.depth)]
        ...
        for block_id, block in enumerate(self.blocks):
            block.register_forward_hook(self.save_output_hooks(block_id))
        self.norm = norm_layer(embed_dim)

        # Classifier head
        n_order = 30 + 1
        self.head = nn.Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.order = nn.Linear(embed_dim, n_order) if n_order > 0 else nn.Identity()
    ...
    def save_output_hooks(self, _id) -> Callable:
        def fn(module, module_in, module_out):
            self.attn_feature[_id] = module.attn_feature
        return fn
...
class dvit_base_patch16_224(nn.Module):
    def __init__(self, cfg, **kwargs):
        super(dvit_base_patch16_224, self).__init__()
        ...
        # Skip Hook
        # self.model.register_forward_hook(self.save_output_hooks())

        self.attention_type = cfg.TIMESFORMER.ATTENTION_TYPE
        self.model.default_cfg = default_cfgs['dvit_base_patch16_224']
    ...
    def forward(self, x):
        x, order = self.model(x)
        return x, order #, #self.attn_feature
...
class DirecFormer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
        ...
        self.attention_type = attention_type
        self.model.default_cfg = default_cfgs['dvit_base_patch'+str(patch_size)+'_224']
